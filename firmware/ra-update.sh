...
# For this, it checks the current flashed firmware information (CURRENT_...)
...
TARGET_PB_VERSION="$(echo "$TARGET_VERSION" | awk -F': *' '/Parameter Block Version/ {print $2}')"
CURRENT_PB_VERSION="$(echo "$CURRENT_VERSION" | awk -F': *' '/Parameter Block Version/ {print $2}')"
...
TARGET_VERSION_ONLY="$(echo "$CMP_TARGET_VERSION" | grep "Version" | awk '{print $3}')"
...
if cur_version_eq "0.1.0"; then
    ...
    # remember the updated pb version
    CURRENT_PB_VERSION="$TARGET_PB_VERSION"
...
if cur_version_gt "0.1.0" && cur_version_le "0.2.2"; then
    ...
    ra-pb-create -O "$TARGET_PB_VERSION" -i "$pbfile_current_yaml" -o "$pbfile_target_bin"
...
# firmware < 0.2.6 had no versioned parameter file
...
# check whether the parameter block versions differ (e.g. due to downgrade)
if [ "$CURRENT_PB_VERSION" != "$TARGET_PB_VERSION" ]; then
    echo "Adapting Parameter Block..."
    ...
    # re-create parameter block - maybe with warnings about unsupported fields
    ra-pb-create -O "$TARGET_PB_VERSION" -i "$pbfile_current_yaml" -o "$pbfile_target_bin"
